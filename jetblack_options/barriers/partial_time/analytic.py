...
def price(
    ...
    if TypeFlag == "cdoA" or TypeFlag == "cuoA": # call down-and out and up-and-out type A
        ...
        return price("coB1", S, X, H, t1, T2, r, b, v)
        ...
        return (
            price("cdoA", S, X, H, t1, T2, r, b, v) -
            S * exp((b - r) * T2) * z5 +
            X * exp(-r * T2) * z1
        )
    elif TypeFlag == "puoA":
        return (
            price("cuoA", S, X, H, t1, T2, r, b, v) -
            S * exp((b - r) * T2) * z6 +
            X * exp(-r * T2) * z2
        )
    elif TypeFlag == "poB1": # put out type B1
        return (
            price("coB1", S, X, H, t1, T2, r, b, v) -
            S * exp((b - r) * T2) * z8 +
            X * exp(-r * T2) * z4 -
            S * exp((b - r) * T2) * z7 +
            X * exp(-r * T2) * z3
        ...
        return (
            price("cdoB2", S, X, H, t1, T2, r, b, v) -
            S * exp((b - r) * T2) * z7 +
            X * exp(-r * T2) * z3
        )
    elif TypeFlag == "puoB2": # put up-and-out type B2
        return (
            price("cuoB2", S, X, H, t1, T2, r, b, v) -
            S * exp((b - r) * T2) * z8 +
            X * exp(-r * T2) * z4
        )
    else:
...
def EPartialTimeBarrier(
    ...
    if OutPutFlag == "p": # Value
        return price(TypeFlag, S, X, H, t1, T2, r, b, v)
    elif OutPutFlag == "d": # Delta
        return (price(TypeFlag, S + dS, X, H, t1, T2, r, b, v) - price(TypeFlag, S - dS, X, H, t1, T2, r, b, v)) / (2 * dS)
    elif OutPutFlag == "dddv": # DeltaDVol
        return 1 / (4 * dS * 0.01) * (price(TypeFlag, S + dS, X, H, t1, T2, r, b, v + 0.01) - price(TypeFlag, S + dS, X, H, t1, T2, r, b, v - 0.01) - price(TypeFlag, S - dS, X, H, t1, T2, r, b, v + 0.01) + price(TypeFlag, S - dS, X, H, t1, T2, r, b, v - 0.01)) / 100
    elif OutPutFlag == "g": # Gamma
        return (price(TypeFlag, S + dS, X, H, t1, T2, r, b, v) - 2 * price(TypeFlag, S, X, H, t1, T2, r, b, v) + price(TypeFlag, S - dS, X, H, t1, T2, r, b, v)) / (dS ** 2)
    elif OutPutFlag == "gp": #  GammaP
        return S / 100 * EPartialTimeBarrier("g", TypeFlag, S + dS, X, H, t1, T2, r, b, v)
    elif OutPutFlag == "gv": # DGammaDvol
        return (price(TypeFlag, S + dS, X, H, t1, T2, r, b, v + 0.01) - 2 * price(TypeFlag, S, X, H, t1, T2, r, b, v + 0.01) + price(TypeFlag, S - dS, X, H, t1, T2, r, b, v + 0.01) - price(TypeFlag, S + dS, X, H, t1, T2, r, b, v - 0.01) + 2 * price(TypeFlag, S, X, H, t1, T2, r, b, v - 0.01) - price(TypeFlag, S - dS, X, H, t1, T2, r, b, v - 0.01)) / (2 * 0.01 * dS ** 2) / 100
    elif OutPutFlag == "v": #  Vega
        return (price(TypeFlag, S, X, H, t1, T2, r, b, v + 0.01) - price(TypeFlag, S, X, H, t1, T2, r, b, v - 0.01)) / 2
        ...
        return (price(TypeFlag, S, X, H, t1, T2, r, b, v + 0.01) - 2 * price(TypeFlag, S, X, H, t1, T2, r, b, v) + price(TypeFlag, S, X, H, t1, T2, r, b, v - 0.01)) / 0.01 ** 2 / 10000
    elif OutPutFlag == "r": # Rho
        return (price(TypeFlag, S, X, H, t1, T2, r + 0.01, b + 0.01, v) - price(TypeFlag, S, X, H, t1, T2, r - 0.01, b - 0.01, v)) / 2
    elif OutPutFlag == "fr": # Futures option Rho
        return (price(TypeFlag, S, X, H, t1, T2, r + 0.01, 0, v) - price(TypeFlag, S, X, H, t1, T2, r - 0.01, 0, v)) / 2
        ...
        if t1 <= 1 / 365:
            return (price(TypeFlag, S, X, H, 0.00001, T2 - 1 / 365, r, b, v) - price(TypeFlag, S, X, H, t1, T2, r, b, v))
        else:
            return (price(TypeFlag, S, X, H, t1 - 1 / 365, T2 - 1 / 365, r, b, v) - price(TypeFlag, S, X, H, t1, T2, r, b, v))
    elif OutPutFlag == "dx": # Strike Delta
        return (price(TypeFlag, S, X + dS, H, t1, T2, r, b, v) - price(TypeFlag, S, X - dS, H, t1, T2, r, b, v)) / (2 * dS)
    elif OutPutFlag == "dxdx": # Strike Gamma
        return (price(TypeFlag, S, X + dS, H, t1, T2, r, b, v) - 2 * price(TypeFlag, S, X, H, t1, T2, r, b, v) + price(TypeFlag, S, X - dS, H, t1, T2, r, b, v)) / (dS ** 2)
    else:
