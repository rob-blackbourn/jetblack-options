...
Public Function EExtremeSpreadOption(OutPutFlag As String, TypeFlag As Integer, S As Double, SMin As Double, SMax As Double, t1 As Double, T As Double, r As Double, b As Double, v As Double, Optional dS)
    ...
    ElseIf OutPutFlag = "gv" Then 'DGammaDVol
        EExtremeSpreadOption = (ExtremeSpreadOption(TypeFlag, S + dS, SMin, SMax, t1, T, r, b, v + 0.01) - 2 * ExtremeSpreadOption(TypeFlag, S, SMin, SMax, t1, T, r, b, v + 0.01) + ExtremeSpreadOption(TypeFlag, S - dS, SMin, SMax, t1, T, r, b, v + 0.01) - ExtremeSpreadOption(TypeFlag, S + dS, SMin, SMax, t1, T, r, b, v - 0.01) + 2 * ExtremeSpreadOption(TypeFlag, S, SMin, SMax, t1, T, r, b, v - 0.01) - ExtremeSpreadOption(TypeFlag, S - dS, SMin, SMax, t1, T, r, b, v - 0.01)) / (2 * 0.01 * dS ^ 2) / 100
    ...
    ElseIf OutPutFlag = "dddv" Then 'DDeltaDvol
        EExtremeSpreadOption = 1 / (4 * dS * 0.01) * (ExtremeSpreadOption(TypeFlag, S + dS, SMin, SMax, t1, T, r, b, v + 0.01) - ExtremeSpreadOption(TypeFlag, S + dS, SMin, SMax, t1, T, r, b, v - 0.01) - ExtremeSpreadOption(TypeFlag, S - dS, SMin, SMax, t1, T, r, b, v + 0.01) + ExtremeSpreadOption(TypeFlag, S - dS, SMin, SMax, t1, T, r, b, v - 0.01)) / 100
    ...
    ElseIf OutPutFlag = "t" Then 'Theta
        If t1 <= 1 / 365 Then
            EExtremeSpreadOption = ExtremeSpreadOption(TypeFlag, S, SMin, SMax, 0.00001, T - 1 / 365, r, b, v) - ExtremeSpreadOption(TypeFlag, S, SMin, SMax, t1, T, r, b, v)
        Else
    ...
    ElseIf OutPutFlag = "s" Then 'Speed
        EExtremeSpreadOption = 1 / dS ^ 3 * (ExtremeSpreadOption(TypeFlag, S + 2 * dS, SMin, SMax, t1, T, r, b, v) - 3 * ExtremeSpreadOption(TypeFlag, S + dS, SMin, SMax, t1, T, r, b, v) + 3 * ExtremeSpreadOption(TypeFlag, S, SMin, SMax, t1, T, r, b, v) - ExtremeSpreadOption(TypeFlag, S - dS, SMin, SMax, t1, T, r, b, v))
    End If

End Function

' Fixed strike lookback options
Public Function FixedStrikeLookback(CallPutFlag As String, S As Double, SMin As Double, SMax As Double, x As Double, T As Double, r As Double, b As Double, v As Double) As Double
    ...
    If CallPutFlag = "c" And x > m Then
        FixedStrikeLookback = S * Exp((b - r) * T) * CND(d1) - x * Exp(-r * T) * CND(d2) + S * Exp(-r * T) * v ^ 2 / (2 * b) * (-(S / x) ^ (-2 * b / v ^ 2) * CND(d1 - 2 * b / v * Sqr(T)) + Exp(b * T) * CND(d1))
    ElseIf CallPutFlag = "c" And x <= m Then
        FixedStrikeLookback = Exp(-r * T) * (m - x) + S * Exp((b - r) * T) * CND(e1) - Exp(-r * T) * m * CND(e2) + S * Exp(-r * T) * v ^ 2 / (2 * b) * (-(S / m) ^ (-2 * b / v ^ 2) * CND(e1 - 2 * b / v * Sqr(T)) + Exp(b * T) * CND(e1))
    ElseIf CallPutFlag = "p" And x < m Then
        FixedStrikeLookback = -S * Exp((b - r) * T) * CND(-d1) + x * Exp(-r * T) * CND(-d1 + v * Sqr(T)) + S * Exp(-r * T) * v ^ 2 / (2 * b) * ((S / x) ^ (-2 * b / v ^ 2) * CND(-d1 + 2 * b / v * Sqr(T)) - Exp(b * T) * CND(-d1))
    ElseIf CallPutFlag = "p" And x >= m Then
        FixedStrikeLookback = Exp(-r * T) * (x - m) - S * Exp((b - r) * T) * CND(-e1) + Exp(-r * T) * m * CND(-e1 + v * Sqr(T)) + Exp(-r * T) * v ^ 2 / (2 * b) * S * ((S / m) ^ (-2 * b / v ^ 2) * CND(-e1 + 2 * b / v * Sqr(T)) - Exp(b * T) * CND(-e1))
    End If

End Function

Public Function EFixedStrikeLookback(OutPutFlag As String, CallPutFlag As String, S As Double, SMin As Double, SMax As Double, x As Double, T As Double, r As Double, b As Double, v As Double, Optional dS)
    ...
    ElseIf OutPutFlag = "gv" Then 'DGammaDVol
        EFixedStrikeLookback = (FixedStrikeLookback(CallPutFlag, S + dS, SMin, SMax, x, T, r, b, v + 0.01) - 2 * FixedStrikeLookback(CallPutFlag, S, SMin, SMax, x, T, r, b, v + 0.01) + FixedStrikeLookback(CallPutFlag, S - dS, SMin, SMax, x, T, r, b, v + 0.01) - FixedStrikeLookback(CallPutFlag, S + dS, SMin, SMax, x, T, r, b, v - 0.01) + 2 * FixedStrikeLookback(CallPutFlag, S, SMin, SMax, x, T, r, b, v - 0.01) - FixedStrikeLookback(CallPutFlag, S - dS, SMin, SMax, x, T, r, b, v - 0.01)) / (2 * 0.01 * dS ^ 2) / 100
    ElseIf OutPutFlag = "gp" Then 'GammaP
    ...
    ElseIf OutPutFlag = "dddv" Then 'DDeltaDvol
        EFixedStrikeLookback = 1 / (4 * dS * 0.01) * (FixedStrikeLookback(CallPutFlag, S + dS, SMin, SMax, x, T, r, b, v + 0.01) - FixedStrikeLookback(CallPutFlag, S + dS, SMin, SMax, x, T, r, b, v - 0.01) - FixedStrikeLookback(CallPutFlag, S - dS, SMin, SMax, x, T, r, b, v + 0.01) + FixedStrikeLookback(CallPutFlag, S - dS, SMin, SMax, x, T, r, b, v - 0.01)) / 100
    ElseIf OutPutFlag = "v" Then 'Vega
    ...
    ElseIf OutPutFlag = "t" Then 'Theta
        If T <= 1 / 365 Then
            EFixedStrikeLookback = FixedStrikeLookback(CallPutFlag, S, SMin, SMax, x, 0.00001, r, b, v) - FixedStrikeLookback(CallPutFlag, S, SMin, SMax, x, T, r, b, v)
        Else
    ...
    ElseIf OutPutFlag = "b" Then 'Carry
        EFixedStrikeLookback = (FixedStrikeLookback(CallPutFlag, S, SMin, SMax, x, T, r, b + 0.01, v) - FixedStrikeLookback(CallPutFlag, S, SMin, SMax, x, T, r, b - 0.01, v)) / (2)
    ElseIf OutPutFlag = "s" Then 'Speed
        EFixedStrikeLookback = 1 / dS ^ 3 * (FixedStrikeLookback(CallPutFlag, S + 2 * dS, SMin, SMax, x, T, r, b, v) - 3 * FixedStrikeLookback(CallPutFlag, S + dS, SMin, SMax, x, T, r, b, v) + 3 * FixedStrikeLookback(CallPutFlag, S, SMin, SMax, x, T, r, b, v) - FixedStrikeLookback(CallPutFlag, S - dS, SMin, SMax, x, T, r, b, v))
...
Public Function FloatingStrikeLookback(CallPutFlag As String, S As Double, SMin As Double, SMax As Double, T As Double, r As Double, b As Double, v As Double) As Double
    ...
    If CallPutFlag = "c" Then
        If b = 0 Then
            FloatingStrikeLookback = S * Exp(-r * T) * CND(a1) - m * Exp(-r * T) * CND(a2) + S * Exp(-r * T) * v * Sqr(T) * (ND(a1) + a1 * (CND(a1) - 1))
        Else
            FloatingStrikeLookback = S * Exp((b - r) * T) * CND(a1) - m * Exp(-r * T) * CND(a2) + Exp(-r * T) * v ^ 2 / (2 * b) * S * ((S / m) ^ (-2 * b / v ^ 2) * CND(-a1 + 2 * b / v * Sqr(T)) - Exp(b * T) * CND(-a1))
        End If
    ElseIf CallPutFlag = "p" Then
        If b = 0 Then
            FloatingStrikeLookback = m * Exp(-r * T) * CND(-a2) - S * Exp(-r * T) * CND(-a1) + S * Exp(-r * T) * v * Sqr(T) * (ND(a1) + CND(a1) * a1)
        Else
            FloatingStrikeLookback = m * Exp(-r * T) * CND(-a2) - S * Exp((b - r) * T) * CND(-a1) + Exp(-r * T) * v ^ 2 / (2 * b) * S * (-(S / m) ^ (-2 * b / v ^ 2) * CND(a1 - 2 * b / v * Sqr(T)) + Exp(b * T) * CND(a1))
        End If
    End If

End Function

Public Function EFloatingStrikeLookback(OutPutFlag As String, CallPutFlag As String, S As Double, SMin As Double, SMax As Double, T As Double, r As Double, b As Double, v As Double, Optional dS)
    ...
    ElseIf OutPutFlag = "gv" Then 'DGammaDVol
        EFloatingStrikeLookback = (FloatingStrikeLookback(CallPutFlag, S + dS, SMin, SMax, T, r, b, v + 0.01) - 2 * FloatingStrikeLookback(CallPutFlag, S, SMin, SMax, T, r, b, v + 0.01) + FloatingStrikeLookback(CallPutFlag, S - dS, SMin, SMax, T, r, b, v + 0.01) - FloatingStrikeLookback(CallPutFlag, S + dS, SMin, SMax, T, r, b, v - 0.01) + 2 * FloatingStrikeLookback(CallPutFlag, S, SMin, SMax, T, r, b, v - 0.01) - FloatingStrikeLookback(CallPutFlag, S - dS, SMin, SMax, T, r, b, v - 0.01)) / (2 * 0.01 * dS ^ 2) / 100
    ElseIf OutPutFlag = "gp" Then 'GammaP
    ...
    ElseIf OutPutFlag = "dddv" Then 'DDeltaDvol
        EFloatingStrikeLookback = 1 / (4 * dS * 0.01) * (FloatingStrikeLookback(CallPutFlag, S + dS, SMin, SMax, T, r, b, v + 0.01) - FloatingStrikeLookback(CallPutFlag, S + dS, SMin, SMax, T, r, b, v - 0.01) - FloatingStrikeLookback(CallPutFlag, S - dS, SMin, SMax, T, r, b, v + 0.01) + FloatingStrikeLookback(CallPutFlag, S - dS, SMin, SMax, T, r, b, v - 0.01)) / 100
    ElseIf OutPutFlag = "v" Then 'Vega
    ...
    ElseIf OutPutFlag = "t" Then 'Theta
        If T <= 1 / 365 Then
            EFloatingStrikeLookback = FloatingStrikeLookback(CallPutFlag, S, SMin, SMax, 0.00001, r, b, v) - FloatingStrikeLookback(CallPutFlag, S, SMin, SMax, T, r, b, v)
        Else
    ...
    ElseIf OutPutFlag = "b" Then 'Carry
        EFloatingStrikeLookback = (FloatingStrikeLookback(CallPutFlag, S, SMin, SMax, T, r, b + 0.01, v) - FloatingStrikeLookback(CallPutFlag, S, SMin, SMax, T, r, b - 0.01, v)) / (2)
    ElseIf OutPutFlag = "s" Then 'Speed
        EFloatingStrikeLookback = 1 / dS ^ 3 * (FloatingStrikeLookback(CallPutFlag, S + 2 * dS, SMin, SMax, T, r, b, v) - 3 * FloatingStrikeLookback(CallPutFlag, S + dS, SMin, SMax, T, r, b, v) + 3 * FloatingStrikeLookback(CallPutFlag, S, SMin, SMax, T, r, b, v) - FloatingStrikeLookback(CallPutFlag, S - dS, SMin, SMax, T, r, b, v))
    End If
...
Public Function EPartialFixedLB(OutPutFlag As String, CallPutFlag As String, S As Double, x As Double, t1 As Double, T As Double, r As Double, b As Double, v As Double, Optional dS)
    ...
    ElseIf OutPutFlag = "gv" Then 'DGammaDVol
        EPartialFixedLB = (PartialFixedLB(CallPutFlag, S + dS, x, t1, T, r, b, v + 0.01) - 2 * PartialFixedLB(CallPutFlag, S, x, t1, T, r, b, v + 0.01) + PartialFixedLB(CallPutFlag, S - dS, x, t1, T, r, b, v + 0.01) - PartialFixedLB(CallPutFlag, S + dS, x, t1, T, r, b, v - 0.01) + 2 * PartialFixedLB(CallPutFlag, S, x, t1, T, r, b, v - 0.01) - PartialFixedLB(CallPutFlag, S - dS, x, t1, T, r, b, v - 0.01)) / (2 * 0.01 * dS ^ 2) / 100
    ElseIf OutPutFlag = "gp" Then 'GammaP
    ...
    ElseIf OutPutFlag = "dddv" Then 'DDeltaDvol
        EPartialFixedLB = 1 / (4 * dS * 0.01) * (PartialFixedLB(CallPutFlag, S + dS, x, t1, T, r, b, v + 0.01) - PartialFixedLB(CallPutFlag, S + dS, x, t1, T, r, b, v - 0.01) - PartialFixedLB(CallPutFlag, S - dS, x, t1, T, r, b, v + 0.01) + PartialFixedLB(CallPutFlag, S - dS, x, t1, T, r, b, v - 0.01)) / 100
    ElseIf OutPutFlag = "v" Then 'Vega
    ...
    ElseIf OutPutFlag = "t" Then 'Theta
        If t1 <= 1 / 365 Then
            EPartialFixedLB = PartialFixedLB(CallPutFlag, S, x, 0.00001, T - 1 / 365, r, b, v) - PartialFixedLB(CallPutFlag, S, x, t1, T, r, b, v)
        Else
    ...
    ElseIf OutPutFlag = "b" Then 'Carry
        EPartialFixedLB = (PartialFixedLB(CallPutFlag, S, x, t1, T, r, b + 0.01, v) - PartialFixedLB(CallPutFlag, S, x, t1, T, r, b - 0.01, v)) / (2)
    ElseIf OutPutFlag = "s" Then 'Speed
        EPartialFixedLB = 1 / dS ^ 3 * (PartialFixedLB(CallPutFlag, S + 2 * dS, x, t1, T, r, b, v) - 3 * PartialFixedLB(CallPutFlag, S + dS, x, t1, T, r, b, v) + 3 * PartialFixedLB(CallPutFlag, S, x, t1, T, r, b, v) - PartialFixedLB(CallPutFlag, S - dS, x, t1, T, r, b, v))
    ElseIf OutPutFlag = "dx" Then 'Strike Delta
...
Public Function PartialFloatLB(CallPutFlag As String, S As Double, SMin As Double, SMax As Double, t1 As Double, T2 As Double, r As Double, b As Double, v As Double, lambda As Double)
...
Public Function EPartialFloatLB(OutPutFlag As String, CallPutFlag As String, S As Double, SMin As Double, SMax As Double, t1 As Double, T As Double, r As Double, b As Double, v As Double, lambda As Double, Optional dS)
    ...
    ElseIf OutPutFlag = "gv" Then 'DGammaDVol
        EPartialFloatLB = (PartialFloatLB(CallPutFlag, S + dS, SMin, SMax, t1, T, r, b, v + 0.01, lambda) - 2 * PartialFloatLB(CallPutFlag, S, SMin, SMax, t1, T, r, b, v + 0.01, lambda) + PartialFloatLB(CallPutFlag, S - dS, SMin, SMax, t1, T, r, b, v + 0.01, lambda) - PartialFloatLB(CallPutFlag, S + dS, SMin, SMax, t1, T, r, b, v - 0.01, lambda) + 2 * PartialFloatLB(CallPutFlag, S, SMin, SMax, t1, T, r, b, v - 0.01, lambda) - PartialFloatLB(CallPutFlag, S - dS, SMin, SMax, t1, T, r, b, v - 0.01, lambda)) / (2 * 0.01 * dS ^ 2) / 100
    ElseIf OutPutFlag = "gp" Then 'GammaP
    ...
    ElseIf OutPutFlag = "dddv" Then 'DDeltaDvol
        EPartialFloatLB = 1 / (4 * dS * 0.01) * (PartialFloatLB(CallPutFlag, S + dS, SMin, SMax, t1, T, r, b, v + 0.01, lambda) - PartialFloatLB(CallPutFlag, S + dS, SMin, SMax, t1, T, r, b, v - 0.01, lambda) - PartialFloatLB(CallPutFlag, S - dS, SMin, SMax, t1, T, r, b, v + 0.01, lambda) + PartialFloatLB(CallPutFlag, S - dS, SMin, SMax, t1, T, r, b, v - 0.01, lambda)) / 100
    ElseIf OutPutFlag = "v" Then 'Vega
    ...
    ElseIf OutPutFlag = "t" Then 'Theta
        If t1 <= 1 / 365 Then
            EPartialFloatLB = PartialFloatLB(CallPutFlag, S, SMin, SMax, 0.00001, T - 1 / 365, r, b, v, lambda) - PartialFloatLB(CallPutFlag, S, SMin, SMax, t1, T, r, b, v, lambda)
        Else
    ...
    ElseIf OutPutFlag = "b" Then 'Carry
        EPartialFloatLB = (PartialFloatLB(CallPutFlag, S, SMin, SMax, t1, T, r, b + 0.01, v, lambda) - PartialFloatLB(CallPutFlag, S, SMin, SMax, t1, T, r, b - 0.01, v, lambda)) / (2)
    ElseIf OutPutFlag = "s" Then 'Speed
        EPartialFloatLB = 1 / dS ^ 3 * (PartialFloatLB(CallPutFlag, S + 2 * dS, SMin, SMax, t1, T, r, b, v, lambda) - 3 * PartialFloatLB(CallPutFlag, S + dS, SMin, SMax, t1, T, r, b, v, lambda) + 3 * PartialFloatLB(CallPutFlag, S, SMin, SMax, t1, T, r, b, v, lambda) - PartialFloatLB(CallPutFlag, S - dS, SMin, SMax, t1, T, r, b, v, lambda))
    End If
