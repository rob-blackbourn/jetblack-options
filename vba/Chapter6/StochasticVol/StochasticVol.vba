...
Public Function HullWhite87SV(CallPutFlag As String, S As Double, X As Double, T As Double, r As Double, b As Double, v As Double, Vvol As Double) As Double
    ...
    cVVV = S * Exp((b - r) * T) * Sqr(T) * ND(d1) * ((d1 * d2 - 1) * (d1 * d2 - 3) - (d1 ^ 2 + d2 ^ 2)) / (8 * v ^ 5)
...
Public Function EHullWhite87SV(OutPutFlag As String, CallPutFlag As String, S As Double, X As Double, T As Double, r As Double, b As Double, v As Double, Vvol As Double, Optional dS)
...
Public Function HullWhite88SV(CallPutFlag As String, S As Double, X As Double, T As Double, r As Double, b As Double, sig0 As Double, sigLR As Double, HL As Double, Vvol As Double, rho As Double) As Double
    ...
    cSVV = S * Exp((b - r) * T) / (4 * Vbar ^ 2) * ND(d1) * (-d1 * d2 ^ 2 + d1 + 2 * d2)
    cVVV = S * Exp((b - r) * T) * ND(d1) * Sqr(T) / (8 * Vbar ^ 2.5) * ((d1 * d2 - 1) * (d1 * d2 - 3) - (d1 ^ 2 + d2 ^ 2))
    ...
    Else ' Beta different from zero
        phi1 = rho ^ 2 / Beta ^ 4 * ((A + Beta * v) * (ed * (delta ^ 2 / 2 - delta + 1) - 1) + A * (ed * (2 - delta) - (2 + delta)))
        phi2 = 2 * phi1 + 1 / (2 * Beta ^ 4) * ((A + Beta * v) * (ed ^ 2 - 2 * delta * ed - 1) - A / 2 * (ed ^ 2 - 4 * ed + 2 * delta + 3))
        phi3 = rho ^ 2 / (2 * Beta ^ 6) * ((A + Beta * v) * (ed - delta * ed - 1) - A * (1 + delta - ed)) ^ 2
        phi4 = 2 * phi3
        f1 = rho / (Beta ^ 3 * T) * ((A + Beta * v) * (1 - ed + delta * ed) + A * (1 + delta - ed)) * cSV
    End If
...
Public Function EHullWhite88SV(OutPutFlag As String, CallPutFlag As String, S As Double, X As Double, T As Double, r As Double, b As Double, sig0 As Double, sigLR As Double, HL As Double, Vvol As Double, rho As Double, Optional dS)
